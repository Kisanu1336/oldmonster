...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\onelove\\configs"), name + ".cfg");
    }
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        onelove.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        onelove.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        for (Style style : onelove.getInstance().getStyleManager().getStyleList()) {
            ...
            styleObject.addProperty("selected", onelove.getInstance().getStyleManager().getCurrentStyle() == style);
