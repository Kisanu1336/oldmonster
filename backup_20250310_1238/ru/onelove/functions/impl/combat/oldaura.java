...
import ru.onelove.utils.player.MouseUtil;
...
@FunctionRegister(name = "oldaura", type = Category.Combat)
public class oldaura extends Function {
    @Getter
    private final ModeSetting type = new ModeSetting("Тип", "Плавная", "Плавная", "Резкая", "Ft");
    ...
    public oldaura(AutoPotion autoPotion) {
        ...
        switch (type.get()) {
            ...
            case "Ft" -> {
                ...
                Vector3d targetPos = new Vector3d(
                        (box.minX + box.maxX) / 2,
                        box.minY + (target.getEyeHeight()),
                        (box.minZ + box.maxZ) / 2);

                float rdYaw = (random.nextFloat() - 0.5f) * 4f;
                float rdPitch = (random.nextFloat() - 0.5f) * 2f;

                float yaw = rotateVector.x + roundedYaw + rdYaw;
                float pitch = clamp(rotateVector.y + pitchDelta + rdPitch, -90, 90);
