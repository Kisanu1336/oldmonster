...
import ru.onelove.events.EventChangeWorld;
import ru.onelove.events.TickEvent;
...
import ru.onelove.functions.impl.player.ItemCooldown;
import ru.onelove.ui.mainmenu.MainScreen;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.unhooked) {
            ...
            this.mainWindow.setWindowTitle("monster client");
        }
    }
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        onelove.getInstance().getEventBus().post(tickEvent);
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            onelove.getInstance().getEventBus().post(player.tick);
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        onelove.getInstance().getEventBus().post(new EventChangeWorld());
    }
