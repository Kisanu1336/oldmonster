...
import ru.onelove.command.CommandDispatcher;
import ru.onelove.command.impl.DispatchResult;
import ru.onelove.events.*;
import ru.onelove.functions.api.FunctionRegistry;
import ru.onelove.functions.impl.misc.AntiPush;
import ru.onelove.functions.impl.misc.SelfDestruct;
import ru.onelove.functions.impl.movement.AutoSprint;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!onelove.getInstance().playerOnServer) {
                onelove.getInstance().playerOnServer = true;
            }

            onelove.getInstance().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private void onUpdateWalkingPlayer() {
        ...
        onelove.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            onelove.getInstance().getEventBus().post(eventMotion);
        ...
        onelove.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = onelove.getInstance().getCommandDispatcher();
    ...
    public void closeScreen() {
        ...
        onelove.getInstance().getEventBus().post(event);
        if (!event.isCancel()) {
    ...
    public void setSprinting(boolean sprinting) {
        onelove.getInstance().getEventBus().post(sprintEvent);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            onelove.getInstance().getEventBus().post(noSlowEvent);
