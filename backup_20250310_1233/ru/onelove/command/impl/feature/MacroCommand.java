package ru.onelove.command.impl.feature;

import ru.onelove.onelove;
import ru.onelove.command.*;
import ru.onelove.command.impl.CommandException;
import ru.onelove.MacroManager;
...
public class MacroCommand implements Command, MultiNamedCommand, CommandWithAdvice {
    ...
    private void removeMacro(Parameters parameters) {
        ...
        onelove.getInstance().getMacroManager().deleteMacro(macroName);
    ...
    private void clearMacros() {
        onelove.getInstance().getMacroManager().clearList();
        logger.log(TextFormatting.GREEN + "Все макросы были удалены.");
    }

    private void printMacrosList() {
        if (onelove.getInstance().getMacroManager().isEmpty()) {
            logger.log(TextFormatting.RED + "Список пустой");
            return;
        }
        onelove.getInstance().getMacroManager().macroList
